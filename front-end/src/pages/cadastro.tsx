...
const Cadastro: React.FC = () => {
  ...
  const handleCadastro = async () => {
    // Atualiza os erros
    const matriculaErro = matricula.length === 0;
    const senhaErro = senha.length < 6;
    const confirmSenhaErro = confirmSenha !== senha;
    const nomeErro = nome.length === 0;
    const emailErro = !email.includes("@");

    setMatriculaError(matriculaErro);
    setSenhaError(senhaErro);
    setConfirmSenhaError(confirmSenhaErro);
    setNomeError(nomeErro);
    setEmailError(emailErro);

    // Apenas envia se não houver erros
    if (
      !matriculaErro &&
      !senhaErro &&
      !confirmSenhaErro &&
      ...
      console.log("Erros nos campos. Verifique os dados antes de enviar.");
    }
  };
