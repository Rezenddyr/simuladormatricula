...
const RenderTable: React.FC<{ title: string; schedule: ScheduleSlot[] }> = ({ title, schedule }) => (
  <Box sx={{ mb: 4, flex: 1, display: 'flex', flexDirection: 'column', mx: 1 }}>
    <Typography variant="h5" sx={{ mb: 2, textAlign: 'center', color: '#FFFFFF' }}>
      {title}
    </Typography>
    <TableContainer component={Paper} sx={{ backgroundColor: '#FFFFFF', borderRadius: '8px' }}>
      <Table>
        <TableHead>
          <TableRow>
            <TableCell sx={{ backgroundColor: '#0085EA', color: '#FFFFFF', textAlign: 'center' }}>
              Horários
            </TableCell>
            {daysOfWeek.map((day) => (
              <TableCell
